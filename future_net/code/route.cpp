...
int update_ans(int load_len,int load[],bool flag = true)
{
	...
	do {
		...
		if(tmp_ans >= tmp_good || is_timeout())
			return tmp_ans;
	} while(1);
}
...
// 起点开始搜索
int dfs(int id, bool inload[], int cnt, bool flag = false)
{
	...
	while(front < rear && find_cnt < find_cnt_limit)
	{
		...
		/*
		if(front > 0 && w[que[front]] != w[que[front-1]])
		{
		...
		*/
		...
		for(int i=0; i<node_next[now_id].size(); i++)
		{
			...
			if(must_node[r_node] || (cnt == 0 && r_node == eid) )
			{
				...
				find_cnt ++;
				int res = dfs(r_node, inload, cnt - 1,flag);
				//find_cnt --;
				ans_sum =  t_ans_sum;
				...
				if(!flag)
				{
					...
					if(cnt > min_l && cnt < must_node_cnt - min_g)
						return fin_res;
					else if(cnt > min_l && cnt < must_node_cnt - min_g + 3 && fin_res > 1)
						return fin_res;
				}
				else
				{
					...
					if(cnt > min_l && cnt < must_node_cnt - min_g)
						return fin_res;
					else if(cnt < must_node_cnt - min_g - 3)
						return fin_res;
				}
...
void search_route(char *topo[5000], int edge_num, char *demand)
{
	...
	bool flag = false;
	if(node_size<=500 && must_node_cnt <=30)
	{
		flag = true;
	}
	
	//flag = false;
	int last_cost = good_cost+1;
	cout << "flag:" << flag << endl; 	
	while(find_cnt_up < tmp_up && good_cost < last_cost)
	{
		...
		int res = dfs(sid,tag,cnt,flag);
		if(res == END_OUT || (now_time - start_time > LIMIT_TIME/2) )
			break;
